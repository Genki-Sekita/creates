...
public class MyOctagonal extends MyDrawing{
	...
	public MyOctagonal(int xpt, int ypt, int wide, int hight, Color lineColor, Color fillColor, int lineWidth){
		super(wide, hight, lineColor, fillColor, lineWidth);
		setLocation(xpt, ypt);
	}
	
	public MyOctagonal(int xpt, int ypt, int wide, int hight){
		super(wide, hight);
		setLocation(xpt, ypt);
	}
	
	public MyOctagonal(int xpt, int ypt, Color lineColor, Color fillColor, int lineWidth){
	...
	public int[] points(int x, int w, boolean xTrue){
		...
		for(int i = 0; i < 8; i++){
			if (xTrue) Points[i] = x + w/2 + (int)( (double)w/2 * Math.sin(Math.PI/4 * i));
			else Points[i] = x + w/2 + (int)( (double)w/2 * Math.cos(Math.PI/4 * i));
		}
		return Points;
	}
	...
	public void draw(Graphics g){
		...
		if(getKage()){
			...
			Polygon polygon1 = new Polygon(xPoints,yPoints, 8);
			g2.fill(polygon1);
			g2.setColor(Color.black);
			Polygon polygon2 = new Polygon(xPoints,yPoints, 8);
			g2.draw(polygon2);
		}
